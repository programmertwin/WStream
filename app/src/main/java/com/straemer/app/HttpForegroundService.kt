...
class HttpForegroundService : Service() {
  ...
  private fun makeNotification(): Notification {
    ...
    if (nm.getNotificationChannel(chanId) == null) {
      nm.createNotificationChannel(NotificationChannel(chanId, "UI XML Stream", NotificationManager.IMPORTANCE_MIN))
    }
    val pending = PendingIntent.getActivity(this, 0, Intent(this, MainActivity::class.java), PendingIntent.FLAG_IMMUTABLE)
    return Notification.Builder(this, chanId).setContentTitle("UI XML Inspector در حال اجرا").setContentText("پورت 8713 آماده است").setSmallIcon(android.R.drawable.stat_sys_data_blink).setContentIntent(pending).build()
  }
  class MiniServer(port: Int) : NanoHTTPD(port) {
    override fun serve(session: IHTTPSession): Response = when (session.uri) {
      "/latest" -> {
        ...
        val etag = "W/\"" + UiDumpAccessibilityService.version.get().toString() + "\"" 
        val inm = session.headers["if-none-match"]
        if (etag == inm) newFixedLengthResponse(Response.Status.NOT_MODIFIED, "text/plain", "").apply { addHeader("ETag", etag) }
        else newFixedLengthResponse(Response.Status.OK, "text/xml; charset=utf-8", xml).apply { addHeader("Cache-Control","no-cache"); addHeader("ETag", etag) }
      }
